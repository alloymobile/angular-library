...
@DataJpaTest
@ActiveProfiles("test")
@DisplayName("RateIlocHistoryRepository Integration Tests")
class RateIlocHistoryRepositoryTest {
    ...
    private RateIlocHistory newHistory(String changeId, LocalDateTime createdOn) {
        RateIlocHistory h = new RateIlocHistory();
        h.setAmountTier(amountTier);
        h.setSubCategory(subCategory);
    ...
    void shouldFindByChangeIdOrderByCreatedOnDesc() {
        ...
        List<RateIlocHistory> list = repository.findByChangeIdOrderByCreatedOnDesc("CHG-HIST");
