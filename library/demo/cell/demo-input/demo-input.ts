...
import { TdInputModel, TdInputType } from '../../../lib/cell/td-input/td-input.model';
import { OutputObject } from '../../../lib/shared/output-object';

type DemoMap = Record<string, any>;
...
const VALIDATION_PRESETS: Record<ValidationPresetKey, Partial<any>> = {
  none: {},
...
function mergeTypeWithValidation(typeCfg: any, preset: ValidationPresetKey): any {
  const p = VALIDATION_PRESETS[preset] || {};
  const next: any = { ...typeCfg, ...p };
...
export class DemoInput {
  ...
  parseError = signal('');
  parsed = signal<any>(mergeTypeWithValidation(BASE_TYPES[this.typeTab()], this.validationTab()));
  ...
  private setConfig(cfg: any): void {
