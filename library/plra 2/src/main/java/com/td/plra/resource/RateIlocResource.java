...
public class RateIlocResource {
    ...
    public ResponseEntity<ApiResponse<PageResponse<RateIlocAdminView>>> findAllDrafts(
        ...
        PageResponse<RateIlocAdminView> page = service.findAllDrafts(allParams, pageable);
    ...
    @Operation(summary = "Approve draft", description = "Approves an ILOC rate draft (PENDING_APPROVAL → APPROVED)")
    @ApiResponses({
    ...
    @Operation(summary = "Activate draft", description = "Activates an approved ILOC rate (APPROVED → ACTIVE). Creates a new Active record and supersedes any existing active rate for the same tier/subcategory.")
    @ApiResponses({
    ...
    public ResponseEntity<ApiResponse<RateIlocAdminView>> activate(
            @Parameter(description = "Draft ID", required = true)
            @PathVariable Long id) {
        log.info("PATCH /api/v1/rates/iloc/drafts/{}/activate", id);
        RateIlocAdminView activated = service.activate(id);
    ...
    public ResponseEntity<ApiResponse<PageResponse<RateIlocAdminView>>> findAllActive(
        ...
        PageResponse<RateIlocAdminView> page = service.findAllActive(allParams, pageable);
        return ApiResponse.ok(page);
    }
