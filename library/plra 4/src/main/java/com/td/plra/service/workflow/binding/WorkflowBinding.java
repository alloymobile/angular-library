...
public class WorkflowBinding extends BaseBinding {
    ...
    public BooleanExpression buildPredicate(Map<String, String> params) {
        ...
        // Global search across changeId, changeBy (OR condition)
        if (hasParam(params, "search")) {
            String searchTerm = getParam(params, "search");
            BooleanExpression searchPredicate = workflow.changeId.containsIgnoreCase(searchTerm)
                    .or(workflow.changeBy.containsIgnoreCase(searchTerm));
