...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, TdLinkBar, TdButtonBar, TdLinkLogo, TdButtonIcon],
...
export class TdNavBarAction implements OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['navBarAction']) {
      // Reserved for future validation or side effects
    }
  }

  onBackButtonOutput(innerOut: OutputObject): void {
    const out = new OutputObject({
      id: this.navBarAction.id,
      type: 'navbar-action',
      action: innerOut.action || 'back-click',
      error: innerOut.error,
      errorMessage: innerOut.errorMessage,
      data: {
        ...innerOut.data,
        source: 'backButton',
        buttonId: this.navBarAction.backButton?.id ?? 'back',
        buttonName: this.navBarAction.backButton?.name ?? 'Back',
      },
    });
