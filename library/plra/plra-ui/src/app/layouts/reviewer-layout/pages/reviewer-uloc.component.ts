...
@Component({
  selector: 'plra-reviewer-uloc', standalone: true,
  imports: [CommonModule, FormsModule, RateMatrixComponent],
  template: `
    <header class="mb-3">
      <h2 class="page-heading"><i class="fa-solid fa-credit-card text-td-green me-2"></i>ULOC — Review</h2>
    </header>
    ...
    @if (tab() === 'view') {
      ...
      <plra-rate-matrix mode="view" productType="ULOC" [tiers]="tiers()" [rows]="activeRows()" [searchTerm]="search" />
    }
    @if (tab() === 'review') {
      <p class="text-muted mb-2" style="font-size:13px"><i class="fa-regular fa-clock me-1"></i>Pending submissions awaiting review</p>
      <plra-rate-matrix mode="view" productType="ULOC" [tiers]="tiers()" [rows]="pendingRows()" />
    }
  `,
  ...
  ngOnInit(): void {
    this.api.getProducts({type:'ULOC',size:1}).subscribe({next: res => {
      ...
      this.api.getAmountTiers(p.name, {unpaged:true}).subscribe({next: tierRes => {
        ...
        this.api.getUlocActive({size:100}).subscribe({next: r => {
          const m = buildViewMatrix(t, r.content, 'ULOC');
          this.tiers.set(m.matrixTiers);
          this.activeRows.set(m.matrixRows);
        }});
        this.api.getUlocDrafts({status:'PENDING',size:100}).subscribe({next: r => {
          const m = buildViewMatrix(t, r.content, 'ULOC');
