...
@Component({
  ...
  template: `
    @if (view() === 'product') {
      <header class="mb-3">
        <h2 class="page-heading"><i class="fa-solid fa-gear text-td-green me-2"></i>ILOC — Admin Settings</h2>
        <p class="page-subtitle">Manage master data for Investment Line of Credit</p>
      </header>
      ...
      @if (product()) {
        <div class="product-card mb-3">
          <div class="d-flex align-items-start justify-content-between mb-3">
            <div>
              <h2 class="product-title"><i class="fa-solid fa-landmark text-td-green me-2"></i>{{ product()!.name }}</h2>
      ...
      <ul class="nav td-tabs mb-3">
        ...
        <li class="nav-item"><button class="nav-link" [class.active]="activeTab() === 'matrix'" (click)="switchTab('matrix')"><i class="fa-solid fa-table-cells me-2"></i>Rate Matrix (SubCat × Tier)</button></li>
      </ul>
      ...
      @if (activeTab() === 'matrix') {
        <div class="d-flex align-items-center justify-content-between mb-3">
          <p class="text-muted mb-0" style="font-size:13px">
            ...
            SubCategory × Amount Tier cross-product. Empty cells = no rate entry exists.
          </p>
          <div class="d-flex gap-2">
            <button class="btn btn-sm btn-outline-success" (click)="loadMatrix()"><i class="fa-solid fa-rotate me-1"></i>Refresh</button>
            <button class="btn btn-sm btn-success" (click)="generateMissingDrafts()" [disabled]="generating()">
              @if (generating()) { <i class="fa-solid fa-spinner fa-spin me-1"></i>Generating... }
              @else { <i class="fa-solid fa-wand-magic-sparkles me-1"></i>Generate Missing Drafts }
            </button>
          </div>
        </div>
        @if (genResult()) {
          <div class="alert mb-3" [class.alert-success]="genResult()!.errors === 0" [class.alert-warning]="genResult()!.errors > 0">
            ...
            @if (genResult()!.total === 0) { All SubCategory × Tier combinations already have rates. }
          </div>
        }
        <plra-rate-matrix mode="view" productType="ILOC" [tiers]="matrixTiers()" [rows]="matrixRows()" />
      }
    }
    ...
    @if (view() === 'subcategory') {
      <header class="mb-3">
        ...
        <nav class="breadcrumb-nav">
          <a (click)="backToProduct()">ILOC Settings</a>
  ...
  styles: [`
    ...
    .td-tabs .nav-link { border:none; background:none; padding:10px 18px; font-size:14px; font-weight:600; color:var(--color-muted); border-bottom:2px solid transparent; margin-bottom:-2px; cursor:pointer; }
