...
@Component({
  selector: 'plra-admin-uloc-rates',
  standalone: true,
  imports: [CommonModule, FormsModule, RateMatrixComponent],
  template: `
    <header class="mb-3">
      <h2 class="page-heading"><i class="fa-solid fa-table-cells text-td-green me-2"></i>ULOC — Administration</h2>
      <p class="page-subtitle">CVP Code × Amount Tier — Enter rates and track approval status</p>
    </header>
    ...
    <!-- ═══ VIEW TAB ═══ -->
    @if (tab() === 'view') {
      <input class="form-control form-control-sm mb-3" style="max-width:300px" placeholder="Search CVP code..." [(ngModel)]="search" />
      <plra-rate-matrix mode="view" productType="ULOC" [tiers]="tiers()" [rows]="viewRows()" [searchTerm]="search" />
    }
    ...
    @if (tab() === 'enter') {
      ...
      <plra-rate-matrix mode="enter" productType="ULOC" [tiers]="tiers()" [rows]="enterRows()" [searchTerm]="search" (onAction)="onEnterAction($event)" />
    }

    <!-- ═══ STATUS TAB ═══ -->
    @if (tab() === 'status') {
      <plra-rate-matrix mode="status" productType="ULOC" [tiers]="tiers()" [rows]="statusRows()" />
    }
    ...
    @if (tab() === 'approve') {
      ...
      <plra-rate-matrix mode="approve" productType="ULOC" [tiers]="tiers()" [rows]="approveRows()" />
    }

    <!-- ═══ HISTORY TAB ═══ -->
    @if (tab() === 'history') {
      <plra-rate-matrix mode="history" productType="ULOC" [tiers]="tiers()" [rows]="historyRows()" />
    }
  `,
...
export class AdminUlocRatesComponent implements OnInit {
  ...
  private loadEnter(): void {
    forkJoin([
      ...
      next: ([activeRes, draftRes]) => {
        const m = buildEnterMatrix(this.tierData, activeRes.content, draftRes.content, 'ULOC');
  ...
  onEnterAction(event: MatrixAction): void {
    if (event.action === 'save-row') {
      ...
      for (const cell of cells) {
        const p = { cvpCodeId: row.id, amountTierId: cell.tierId, targetRate: cell.newRate!, floorRate: cell.newFloor ?? cell.newRate!, startDate: row.effectiveDate || new Date().toISOString().substring(0,10), expiryDate: row.expiryDate || new Date(Date.now()+365*86400000).toISOString().substring(0,10), notes: cell.discretion ?? '' };
  ...
  private loadApprove(): void {
    forkJoin([
      ...
      next: ([activeRes, pendingRes]) => {
        const m = buildApproveMatrix(this.tierData, activeRes.content, pendingRes.content, 'ULOC');
    ...
    for (const row of rows) {
      for (const draftId of row.draftIds) {
        ...
        const obs = row.decision === 'APPROVE'
          ? this.api.approveUlocDraft(draftId, row.comment || undefined)
          : this.api.rejectUlocDraft(draftId, row.comment || undefined);
  ...
  private loadStatus(): void {
    this.api.getUlocDrafts({ size: 500 }).subscribe({
      next: r => {
        const decided = r.content.filter(d => d.status !== 'DRAFT' && d.status !== 'CANCELLED');
        const m = buildStatusMatrix(this.tierData, decided, 'ULOC');
  ...
  private loadHistory(): void {
    forkJoin([
      this.api.getUlocHistory({ size: 500 }),
      ...
      next: ([histRes, draftRes]) => {
        ...
        const m = buildStatusMatrix(this.tierData, allHistorical, 'ULOC');
