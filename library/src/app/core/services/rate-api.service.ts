...
export class RateApiService {
  ...
  reviewIlocActive(id: number, message?: string): Observable<void> {
    return this.http.patch(`${this.base}/rates/iloc/active/${id}/review`, null, { params: this.buildParams(message ? { message } : undefined) }).pipe(map(() => void 0));
  }
  ...
  reviewUlocActive(id: number, message?: string): Observable<void> {
    return this.http.patch(`${this.base}/rates/uloc/active/${id}/review`, null, { params: this.buildParams(message ? { message } : undefined) }).pipe(map(() => void 0));
  }
  ...
  private get<T>(url: string, params?: Record<string, any>): Observable<T> {
    return this.http.get<ApiResponse<T>>(url, { params: this.buildParams(params) }).pipe(
      tap(() => console.log("[PLRA] GET", url)),
      map(r => r.data),
      catchError(err => { console.error("[PLRA] GET FAILED", url, err?.status, err?.error ?? err); return throwError(() => err); })
    );
  }

  private post<T>(url: string, body: any): Observable<T> {
    return this.http.post<ApiResponse<T>>(url, body).pipe(
      tap(() => console.log("[PLRA] POST", url)),
      map(r => r.data),
      catchError(err => { console.error("[PLRA] POST FAILED", url, err?.status, err?.error ?? err); return throwError(() => err); })
    );
  }

  private put<T>(url: string, body: any): Observable<T> {
    return this.http.put<ApiResponse<T>>(url, body).pipe(
      tap(() => console.log("[PLRA] PUT", url)),
      map(r => r.data),
      catchError(err => { console.error("[PLRA] PUT FAILED", url, err?.status, err?.error ?? err); return throwError(() => err); })
    );
  }

  private patch<T>(url: string, params?: Record<string, any>): Observable<T> {
    return this.http.patch<ApiResponse<T>>(url, null, { params: this.buildParams(params) }).pipe(
      tap(() => console.log("[PLRA] PATCH", url)),
      map(r => r.data),
      catchError(err => { console.error("[PLRA] PATCH FAILED", url, err?.status, err?.error ?? err); return throwError(() => err); })
    );
  }

  private del(url: string): Observable<void> {
    return this.http.delete<ApiResponse<void>>(url).pipe(
      tap(() => console.log("[PLRA] DELETE", url)),
      map(() => void 0),
      catchError(err => { console.error("[PLRA] DELETE FAILED", url, err?.status, err?.error ?? err); return throwError(() => err); })
    );
  }
}
