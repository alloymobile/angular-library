...
import { MatrixTier, MatrixRow, MatrixCell } from './rate-matrix/rate-matrix';
...
export function emptyRow(id: number, name: string, tiers: MatrixTier[]): MatrixRow {
  return { id, name, draftIds: [], cells: tiers.map(t => emptyCell(t.id)), effectiveDate: defaultEffectiveDate(), expiryDate: defaultExpiryDate(), status: '', selected: false, submittedBy: '', submittedOn: '', decision: 'APPROVE', comment: '', result: '', decisionBy: '', decisionOn: '', changedBy: '', changedOn: '' };
}
...
export function buildViewMatrix(
  ...
  for (const rate of rates) {
    ...
    if (fmt(rate.startDate)) row.effectiveDate = fmt(rate.startDate);
    if (fmt(rate.expiryDate)) row.expiryDate = fmt(rate.expiryDate);
...
export function buildEnterMatrix(
  ...
  for (const active of activeRates) {
    ...
    if (fmt(active.startDate)) row.effectiveDate = fmt(active.startDate);
    if (fmt(active.expiryDate)) row.expiryDate = fmt(active.expiryDate);
    row.status = 'ACTIVE';
  }
  ...
  for (const draft of draftRates) {
    ...
    if (fmt(draft.startDate)) row.effectiveDate = fmt(draft.startDate);
    if (fmt(draft.expiryDate)) row.expiryDate = fmt(draft.expiryDate);
  }
...
export function buildApproveMatrix(
  ...
  for (const pending of pendingRates) {
    ...
    if (fmt(pending.startDate)) row.effectiveDate = fmt(pending.startDate);
    if (fmt(pending.expiryDate)) row.expiryDate = fmt(pending.expiryDate);
  }
  return { matrixTiers, matrixRows: [...grouped.values()] };
}
