...
@Component({
  selector: 'plra-rate-status-filter',
  standalone: true,
  imports: [CommonModule, TdInput, TdSearch],
  templateUrl: './rate-status-filter.html',
  styleUrls: ['./rate-status-filter.css']
})
export class RateStatusFilterComponent implements OnInit, OnChanges {
  t = inject(TranslationService);
  private api = inject(RateApiService);

  @Input() productName = '';
  @Input() productType: 'ULOC' | 'ILOC' = 'ULOC';
  @Input() availableStatuses: string[] = ['DRAFT', 'PENDING', 'APPROVED', 'REJECTED'];
  @Output() filterChange = new EventEmitter<StatusFilterState>();
  ...
  private state: StatusFilterState = { search: '', categoryId: '', subCategoryIds: [], statuses: [] };
  private allStatuses: string[] = [];

  ngOnInit(): void { this.buildModels(); }

  ngOnChanges(changes: SimpleChanges): void {
    if (changes['productName'] && this.productName) { this.loadCategories(); }
    if (changes['availableStatuses']) { this.buildStatusInput(); }
  }

  private buildModels(): void {
    this.searchModel = new TdSearchModel({
      id: 'status-filter-search', className: 'mb-0', minChars: 1, debounceMs: 300,
    ...
    this.buildStatusInput();

    if (this.productName) { this.loadCategories(); }
  }

  private buildStatusInput(): void {
    this.allStatuses = [...this.availableStatuses];
    this.state.statuses = [...this.allStatuses];
    const opts = this.allStatuses.map(s => ({ value: s, label: s }));
    this.statusInput = new TdInputModel({
      name: 'statuses', type: 'checkselect', label: '', className: 'form-select form-select-sm',
      options: opts, value: [...this.allStatuses]
    });
  }
  ...
  clearAll(): void {
    this.state = { search: '', categoryId: '', subCategoryIds: [], statuses: [...this.allStatuses] };
    this.buildModels();
    this.emit();
  }
