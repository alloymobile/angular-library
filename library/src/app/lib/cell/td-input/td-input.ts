...
export class TdInput implements OnChanges {
  ...
  uploading = false;
  checkselectOpen = false;

  get checkselectLabel(): string {
    if (!this.model || !this.model.options) return '';
    const selected = Array.isArray(this.val) ? this.val : [];
    if (selected.length === 0 || selected.length === this.model.options.length) {
      return 'All selected';
    }
    if (selected.length === 1) {
      const opt = this.model.options.find(o => o.value === selected[0]);
  ...
  onDocumentClick(event: Event): void {
    ...
    if (!el.closest('.td-checkselect-wrap')) {
      this.checkselectOpen = false;
    }
  }
  ...
  toggleCheckselectDropdown(): void {
    if (this.disabled) return;
    this.checkselectOpen = !this.checkselectOpen;
  }

  onCheckselectToggle(value: string): void {
    const prev = Array.isArray(this.val) ? [...this.val] : [];
    const idx = prev.indexOf(value);
    if (idx > -1) prev.splice(idx, 1);
    else prev.push(value);
  ...
  isCheckselectChecked(value: string): boolean {
