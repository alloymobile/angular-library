...
@Component({
  ...
  templateUrl: './admin-uloc-rates.html',
  styleUrls: ['./admin-uloc-rates.css']
})
export class AdminUlocRatesComponent implements OnInit {
      ...
      next: res => {
        ...
        this.api.getAmountTiers(p.name, { unpaged: true }).subscribe({
          next: tierRes => {
            ...
            this.api.getCvpCodes({ size: 500 }).subscribe({ next: (cvpRes: any) => {
              this.allCvps = cvpRes.content;
              this.loadView();
              this.loadEnter();
  ...
  private loadView(): void {
    ...
    this.api.getUlocActive(params).subscribe({
      next: (r: any) => {
        ...
        this.viewRows.set(buildViewMatrix(this.tierData, r.content, 'ULOC').matrixRows);
      }
    });
  }
  ...
  private loadEnter(): void {
    let cvps = [...this.allCvps];
    const subIds = this.statusFilterParams['subCategoryIds'];
    if (subIds) {
      const ids = subIds.split(',').map((s: string) => Number(s));
      cvps = cvps.filter((c: any) => ids.includes(c.subCategory?.id));
    }
    this.enterTotal = cvps.length;
    this.enterPages = Math.ceil(cvps.length / this.enterSize) || 1;
    if (this.enterPage >= this.enterPages) this.enterPage = Math.max(0, this.enterPages - 1);
    const start = this.enterPage * this.enterSize;
    const pageCvps = cvps.slice(start, start + this.enterSize);
    if (pageCvps.length === 0) { this.enterRows.set([]); return; }
    const cvpIds = pageCvps.map((c: any) => c.id).join(',');
    forkJoin([
      this.api.getUlocActive({ cvpCodeIds: cvpIds, size: 500 }),
      this.api.getUlocDrafts({ cvpCodeIds: cvpIds, status: 'DRAFT', size: 500 })
    ]).subscribe({
      next: ([activeRes, draftRes]: any[]) => {
        const entities = pageCvps.map((c: any) => ({ id: c.id, name: c.name }));
        this.enterRows.set(buildEnterMatrix(this.tierData, activeRes.content, draftRes.content, 'ULOC', entities).matrixRows);
      }
    });
  }
  ...
  saveDrafts(): void {
    ...
    for (const row of rows) {
      for (const cell of row.cells) {
        if (cell.newRate !== null && cell.newRate !== undefined) {
          ...
          // Compute floor = newRate - (discBps / 100)
    ...
    for (const item of toSave) {
      const payload = { cvpCodeId: item.entityId, amountTierId: item.tierId, targetRate: item.targetRate, floorRate: item.floorRate, discretion: item.discretion, startDate: item.startDate, expiryDate: item.expiryDate, notes: item.notes };
  ...
  private loadStatus(): void {
    ...
    forkJoin([
      ...
      next: ([draftRes, wfRes]: any[]) => {
        ...
        const wfs = wfRes.content.filter((w: any) => w.rateType === 'ULOC');
        this.statusRows.set(buildStatusMatrix(this.tierData, filtered, 'ULOC', wfs).matrixRows);
      }
    });
  }
  ...
  private loadApprove(): void {
    ...
    forkJoin([
      this.api.getUlocActive({ size: 500, ...this.statusFilterParams }),
      this.api.getUlocDrafts(p)
    ]).subscribe({
