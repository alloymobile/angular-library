...
export class AdminUlocRatesComponent implements OnInit {
  ...
  private readonly HIST_SORT_MAP: Record<string, string> = { name: 'cvpCode.name', tier: 'amountTier.name', targetRate: 'targetRate', floorRate: 'floorRate', startDate: 'startDate', expiryDate: 'expiryDate', status: 'status', notes: 'notes', createdBy: 'createdBy', createdOn: 'createdOn' };
  ...
  private loadEnter(): void {
    ...
    if (subIds) {
      ...
      cvps = cvps.filter((c: any) => ids.includes(c.subCategoryId));
    }
  ...
  private loadHistory(): void {
    ...
    this.api.getUlocHistory(params).subscribe({
      next: (res: any) => {
