...
-- ===========================
-- 1. REMOVED: All NOTIFICATION_ID indexes on rate tables (6 removed)
-- 2. REMOVED: All WORKFLOW_ID indexes on rate tables (6 removed)
-- 3. REMOVED: PRIME table indexes (IDX_PRIME_RATE, IDX_PRIME_ACTIVE)
-- 4. REMOVED: NOTIFICATION table indexes (IDX_NOTIF_STATUS, etc.)
-- 5. REMOVED: History archive column indexes (ORIGINAL_ID, ARCHIVE_REASON,
--             ARCHIVED_ON, ARCHIVED_BY)
-- 6. UPDATED: Covering indexes adjusted for new column structure
-- Total indexes: ~150 -> 140
--
-- ============================================================================
...
-- Note: ID is PK + FK (UK FK) - PK index covers the FK lookup
...
CREATE INDEX IDX_ILOC_DRF_CHANGEID ON RATE_ILOC_DRAFT (CHANGE_ID);
CREATE INDEX IDX_ILOC_ACT_CHANGEID ON RATE_ILOC_ACTIVE (CHANGE_ID);
CREATE INDEX IDX_ILOC_HST_CHANGEID ON RATE_ILOC_HISTORY (CHANGE_ID);

CREATE INDEX IDX_ULOC_DRF_CHANGEID ON RATE_ULOC_DRAFT (CHANGE_ID);
CREATE INDEX IDX_ULOC_ACT_CHANGEID ON RATE_ULOC_ACTIVE (CHANGE_ID);
CREATE INDEX IDX_ULOC_HST_CHANGEID ON RATE_ULOC_HISTORY (CHANGE_ID);
